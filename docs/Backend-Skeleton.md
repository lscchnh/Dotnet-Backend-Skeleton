...
- Enable **HealthChecks**. To do so,
  ...
  - then add the healthcheck endpoint.
    ...
    For more info about Health checks in .NET, see [here](https://docs.microsoft.com/en-us/aspnet/core/host-and-deploy/health-checks?view=aspnetcore-7.0).
...
- Configure **Swagger/OpenApi** documentation. To do so,
  - Add the following lines in Program.cs class :
    ...
    For more info about Swagger/OpenAPI see [here](https://docs.microsoft.com/en-us/aspnet/core/tutorials/getting-started-with-swashbuckle?view=aspnetcore-7.0&tabs=visual-studio).
...
4. In the new solution folder, create a new Test Project in .NET 7.0.
